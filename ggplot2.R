...
# colocando objetos
p1 <- ggplot(data = green_data, mapping = aes(x = ICE_index, y = GCI_index)) +
  geom_point()+
  geom_text(label = green_data$STATE, color = "black", size = 4) +
  geom_point()+
  geom_smooth(method = lm, se = FALSE, fullrange = T)

p1

p2 <- ggplot(data = green_data, mapping = aes(x = ICE_rank, y = GCI_rank)) +
  geom_point()+
  geom_text(label = green_data$STATE, color = "black", size = 4) +
  geom_point()+
  geom_smooth(method = lm, se = FALSE, fullrange = T)
...
p3 <- ggplot(data = green_data, mapping = aes(x = LPIBE, y = GCI_index)) +
  geom_point()+
  geom_text(label = green_data$STATE, color = "black", size = 4) +
  geom_point()+
  geom_smooth(method = lm, se = FALSE, fullrange = T)
